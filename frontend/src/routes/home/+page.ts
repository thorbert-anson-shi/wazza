  ...
  return {
    quizzes: Promise.all([
      new Promise<QuizDetails[]>((resolve, reject) =>
        resolve(
          quizzes.filter((quiz) => quiz.title.startsWith("J")) as QuizDetails[],
        )
      ),
      new Promise<QuizDetails[]>((resolve, reject) =>
        resolve(
          quizzes.filter((quiz) => quiz.title.length > 16) as QuizDetails[],
        )
      ),
      new Promise<QuizDetails[]>((resolve, reject) =>
        resolve(
          quizzes.filter((quiz) => quiz.title.startsWith("I")) as QuizDetails[],
        )
      ),
